<script setup lang="ts">
  ...
  function confirmClear() {
    ...
    for (const key of Object.keys(localStorage)) {
      if (key.startsWith('vue-news:') || key === 'vueuse-color-scheme') localStorage.removeItem(key)
    }
...
<template>
  <div class="p-4 space-y-4">
    ...
    <div
      class="rounded-xl bg-white shadow-sm overflow-hidden divide-gray-100 divide-y dark:bg-gray-800 dark:divide-gray-700"
    >
    ...
    <div class="px-4 py-3.5 rounded-xl bg-white shadow-sm dark:bg-gray-800">
      <p class="text-xs text-gray-400 leading-6 dark:text-gray-500">
        缓存数据包含收藏 {{ favoriteIds.length }} 条、阅读历史 {{ historyIds.length }} 条、消息
        {{ appMessages.length }} 条、关注 {{ followedUsers.length }} 个、登录状态等，全部仅保存在本机。
      </p>
    </div>

    <p v-if="cleared" class="text-xs text-green-500 text-center">
      <span class="i-carbon-checkmark align-text-bottom inline-block" />
      缓存数据已清除，已恢复默认设置
    </p>
    ...
    <AppDialog :open="clearConfirm" title="清除缓存" @close="clearConfirm = false">
      ...
      <div class="mt-4 flex gap-3">
        ...
        <button class="btn flex-1 !bg-red-500 hover:!bg-red-600" @click="confirmClear">确定清除</button>
      </div>
    </AppDialog>
  </div>
</template>
