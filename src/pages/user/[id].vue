...
<template>
  <div class="pb-6">
    <div
      class="text-white p-5 flex gap-4 items-center"
      :style="{ background: `linear-gradient(135deg, ${gradient[0]}, ${gradient[1]})` }"
    >
      <Avatar :char="username.charAt(0)" :gradient="gradient" size="xl" />
      <div class="flex-1 min-w-0">
        <div class="flex gap-2 items-center">
          <p class="text-lg font-bold truncate">
            {{ username }}
          </p>
          <span v-if="isSelf" class="text-xs px-2 py-0.5 rounded-full bg-white/20 shrink-0">这是你</span>
        </div>
    ...
    >
      <StatItem :highlight="!isSelf" label="关注" :value="followedUsers.length" />
    ...
    <div class="mx-4 mt-4">
      <div class="mb-3 px-1 flex items-center justify-between">
        <h2 class="text-base font-bold">TA 的发布</h2>
        <span class="text-xs text-gray-400">{{ published.length }} 篇</span>
      </div>
      <NewsList emptyIcon="i-carbon-document" emptyText="TA 还没有发布内容" featured :items="published" />
    </div>

    <div class="mx-4 mt-6">
      <div class="mb-3 px-1 flex items-center justify-between">
        <h2 class="text-base font-bold">TA 的热门评论</h2>
        <span class="text-xs text-gray-400">{{ userComments.length }} 条</span>
      </div>
