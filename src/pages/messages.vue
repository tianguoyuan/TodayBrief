...
const typeMeta: Record<string, { icon: string, color: string }> = {
  like: { icon: 'i-carbon-thumbs-up', color: 'text-pink-500' },
  reply: { icon: 'i-carbon-chat', color: 'text-blue-500' },
  follow: { icon: 'i-carbon-user', color: 'text-teal-500' },
  system: { icon: 'i-carbon-information', color: 'text-orange-500' },
}
...
function openMessage(message: AppMessage) {
  markRead(message.id)
  if (message.newsId)
    router.push(`/news/${message.newsId}`)
  else if (message.username)
    router.push(`/user/${message.username}`)
}
  ...
  <div class="p-4">
    <div v-if="appMessages.length" class="mb-3 px-1 flex items-center justify-between">
      <h2 class="text-base font-bold">
        通知中心
        <span v-if="unreadCount" class="text-[10px] text-white ml-1 px-1.5 py-0.5 align-middle rounded-full bg-orange-500">
          {{ unreadCount }}
        </span>
      </h2>
      <div class="flex gap-3 items-center">
        <button v-if="unreadCount" class="text-xs text-gray-400 transition-colors hover:text-orange-500" @click="markAllRead">
          全部已读
        </button>
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <div class="flex gap-2 items-center">
            <p class="text-sm font-medium" :class="isRead(message.id) ? 'text-gray-500 dark:text-gray-400' : 'text-gray-900 dark:text-gray-100'">
              {{ message.title }}
            </p>
