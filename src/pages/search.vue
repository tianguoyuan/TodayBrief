...
<template>
  <div>
    <div class="px-4 pt-3">
      <div class="px-3 py-2 rounded-full bg-white flex gap-2 shadow-sm items-center dark:bg-gray-800">
        ...
        <button
          v-if="query"
          aria-label="清空"
          class="icon-btn text-lg text-gray-400"
          title="清空"
          type="button"
          @click="query = ''"
        >
    ...
    <div v-if="!query" class="p-4">
      <p class="text-xs text-gray-400 mb-2 px-1">热门标签</p>
      <div class="flex flex-wrap gap-2">
        <TagChip v-for="keyword in hotKeywords" :key="keyword" :text="`# ${keyword}`" :to="`/tag/${keyword}`" />
      </div>

      <p class="text-xs text-gray-400 mb-2 mt-6 px-1">大家都在搜</p>
      <NewsList :items="allNews.slice(0, 3)" />
    </div>

    <div v-else class="p-4 pt-3">
      <p class="text-xs text-gray-400 mb-3 px-1">"{{ query }}" 共找到 {{ results.length }} 条结果</p>
      <NewsList emptyIcon="i-carbon-search" emptyText="没有找到相关资讯，换个关键词试试" :items="results" />
    </div>
  </div>
</template>
